...
class EnVariationalDiffusion_2(torch.nn.Module):
    """
    ...
    def sample_normal(self, mu, sigma, node_mask, fix_noise=False):
        """Samples from a Normal distribution N(mu, sigma)."""
        bs = 1 if fix_noise else mu.size(0)
        eps = self.sample_combined_position_feature_noise(bs, mu.size(1), node_mask)
        return mu + sigma * eps

    def log_pxh_given_z0_without_constants(
            self, x, h, z_t, gamma_0, eps, net_out, node_mask, epsilon=1e-10, egnn_f=None):
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always, edges=None):
        ...
        # Combining the terms. t0_always is True only for inference.
        if t0_always:
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, edges=None):
        ...
        # In actuality we do not normalize the coordinates, but only the h.
        x, h, delta_log_px = self.normalize(x, h, node_mask)

        # Reset delta_log_px if not vlb objective.
        # Why actually is the delta_log_px reset?
        if self.training and self.loss_type == 'l2':
        ...
        # print(f"delta_log_px: {delta_log_px}")
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        z_x = utils.sample_center_gravity_zero_gaussian_with_mask(
            ...
            node_mask=node_mask)
        # z_x:  (n_samples, n_nodes, self.n_dims)
        z_h = utils.sample_gaussian_with_mask(
            size=(n_samples, n_nodes, self.in_node_nf), device=node_mask.device,
            node_mask=node_mask)
        # z_h:  (n_samples, n_nodes, self.in_node_nf)
