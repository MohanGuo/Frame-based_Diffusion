...
class GCL(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, normalization_factor, aggregation_method,
        ...
        # edges_in_d=2 without sin_embedding, which is [distance, normalized_distance]
        ...
        if self.attention:
            self.att_mlp = nn.Sequential(
                ...
                nn.Sigmoid())
    # edge_attr is the distance / radial
...
class EquivariantUpdate(nn.Module):
    ...
    def coord_model(self, h, coord, edge_index, coord_diff, edge_attr, edge_mask):
        ...
        # print(f"Shape of output of coord_mlp: {self.coord_mlp(input_tensor).shape}")
        if self.tanh:
...
class EquivariantBlock(nn.Module):
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, edge_attr=None):
        ...
        # the input into the coordinal update is the h, the output of the node update
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
        ...
        # out_node_nf is in_node_nf, which is the dimension of the input h.
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None):
        ...
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)
        h = self.embedding(h)
        #### edge_attr is actually [distances, distances]
        for i in range(0, self.n_layers):
...
def test_egnn():
    ...
    h = torch.randn(batch_size * n_nodes, in_node_nf)
    ...
    try:
        h_out, x_out = model(h, x, edges, node_mask=node_mask, edge_mask=edge_mask)
        print("\nSuccess!")
        print("Output shapes:")
        print(f"Node features: {h_out.shape}")
        print(f"Coordinates: {x_out.shape}")
